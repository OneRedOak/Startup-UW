...
/*-----------------------------------------------------------------------------------*/
	...
	if ( have_comments() ) :
        ...
        if ( ! empty($comments_by_type['comment']) ) : // if there are normal comments ?>
	        ...
	        <h4 class="md-special-heading"><?php comments_number(__('0 Comments', MD_THEME_NAME), __('1 Comment', MD_THEME_NAME), __('% Comments', MD_THEME_NAME)); ?></h4>
        ...
        if ( ! empty($comments_by_type['pings']) ) : // if there are pings ?>
    		...
    		<h3 class="pings-title"><?php _e('Trackbacks for this post', MD_THEME_NAME) ?></h3>
    ...
    <?php else: ?>
            <h4 class="md-special-heading"><?php _e('No comments, write the first!', MD_THEME_NAME); ?></h3>
    <?php endif; ?>
...
<?php 
	...
	if ( comments_open() ) :
	    ...
	    $fields = array(
            ...
            'logged_in_as' => '<p class="logged-in-as">' . sprintf( __( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out &raquo;</a>', MD_THEME_NAME ), admin_url( 'profile.php' ), $user_identity, wp_logout_url( apply_filters( 'the_permalink', get_permalink( ) ) ) ) . '</p>',
            'comment_notes_before' => '',
            'comment_notes_after' => '',
            'title_reply' => __('Leave a Reply', MD_THEME_NAME),
            'title_reply_to' => __('Leave a Reply to %s', MD_THEME_NAME),
            'cancel_reply_link' => __('/ Cancel Reply', MD_THEME_NAME),
