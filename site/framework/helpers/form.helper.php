...
class FORM_HELPER{
	...
	function __construct($post_id, $metabox_fields, $taxonomy = false){
	...
	public function metabox_init(){
		...
		echo '<div id="mt-metabox">';
			echo '<h2 id="mt-title" class="hndle">Advanced Page Options</h2>';

			echo '<ul id="mt-tabs">';
			foreach ($this->metabox_fields as $metabox):

				$class = (isset($metabox['class'])) ? ' class="'.$metabox['class'].'"' : '';

				echo '<li'.$class.'><a href="#"><i class="icon-'.$metabox['icon'].'"></i>'.$metabox['title'].'</a></li>';
			endforeach;
			echo '</ul>';

			foreach ($this->metabox_fields as $metabox):
				echo '<div class="mt-panel">';
	...
	function upload($args){
		...
		echo '<div class="field" data-path="'.$path.'" data-media="'.$args['media'].'">';
		echo '<label>'.$args['label'].'</label>';
		echo '<div class="input">';
	...
	function gallery($args){
		...
		echo '<div class="field" data-path="'.$path.'">';
		echo '<label>'.$args['label'].'</label>';
		echo '<div class="input">';
		echo '<input type="hidden" name="'.$args['name'].'" value="'.$args['value'].'" class="upload_field custom" />';
	...
	function toggle($args){
		($args['value']) ? $checked = ' checked' : $checked = '';
		($args['value']) ? $toggle = ' active' : $toggle = '';
		
		echo '<div class="field toggled">';
		echo '<label>'.$args['label'].'</label>';
		echo '<div class="input">';
